...
with tab2:
    ...
    disp.columns = ["Dimensión","Tipo","Media","Escala","Índice Riesgo","Nivel"]
...
with tab3:
    ...
    fig_corr = px.imshow(corr_mat, x=dims_all, y=dims_all,
                         ...
                         labels={"x":"Dimensión","y":"Dimensión","color":"ρ"})
...
with tab4:
    ...
    with cv1:
        ...
        fig_h = px.imshow(pivot, color_continuous_scale=["#0f172a", NEON["orange"], NEON["red"]],
                          ...
                          labels={"x":"Dimensión","y":"Tipo de cargo","color":"Score norm."})
    ...
    with cv2:
        ...
        for dim in DIMS:
            ...
            for sector in df_f["Sector"].unique():
                ...
                if len(sub) >= 5:
                    ms_rows.append({"Dimensión":dim,"Sector":sector,
                                    "Score":round((sub.mean()-mn)/(mx-mn),3)})
        ms_df = pd.DataFrame(ms_rows)
        fig_ms = px.bar(ms_df, x="Dimensión", y="Score", color="Sector", barmode="group",
                        color_discrete_sequence=[NEON["cyan"], NEON["purple"], NEON["pink"]],
        ...
        fig_ms.update_layout(xaxis_title="Dimensión psicosocial",
...
with tab5:
    ...
    with hg2:
        ...
        comp_df = pd.DataFrame({
            "Dimensión":["Burnout","Desgaste","Somatización","Satisfacción","Comp. Líder","Int. Retiro"],
            "Score":[(bu_m-1)/4,(dl_m-1)/6,(df_f["score_SOM"].mean()-1)/6,
                     (sat_m-1)/6,(cl_m-1)/4,(ir_m-1)/6],
        })
        fig_comp = px.bar(comp_df, x="Score", y="Dimensión", orientation="h",
                          color="Dimensión",
                          color_discrete_sequence=[NEON["red"],NEON["orange"],NEON["pink"],
                                                   NEON["green"],NEON["cyan"],NEON["purple"]],
                          labels={"Score":"Score normalizado (0–1)","Dimensión":""})
        fig_comp.update_traces(marker_line_width=0)
        fig_comp.update_layout(showlegend=False,
                               xaxis_title="Score normalizado (0–1)", yaxis_title="Dimensión")
